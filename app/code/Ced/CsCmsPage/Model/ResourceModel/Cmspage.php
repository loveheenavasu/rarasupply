...
class Cmspage extends  \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('ced_cscmspage_vendor_cmspage', 'page_id');
    }
    ...
    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('ced_cscmspage_vendor_cmspage'), $condition);
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $table = $this->getTable('ced_cscmspage_vendor_cmspagestore');
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['ced_cscmspage_vendor_cmspagestore' => $this->getTable('ced_cscmspage_vendor_cmspagestore')],
                $this->getMainTable() . '.page_id = ced_cscmspage_vendor_cmspagestore.page_id',
                []
            )->where(
                'is_active = ?',
                1
            )->where(
                'ced_cscmspage_vendor_cmspagestore.store_id IN (?)',
                $storeIds
            )->order(
                'ced_cscmspage_vendor_cmspagestore.store_id DESC'
            )->limit(
                1
            );
        }
    ...
    protected function _getLoadByIdentifierSelect($identifier, $store, $isActive = null)
    {
        $select = $this->getConnection()->select()->from(
            ...
            ['cps' => $this->getTable('ced_cscmspage_vendor_cmspagestore')],
    ...
    public function lookupStoreIds($pageId)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('ced_cscmspage_vendor_cmspagestore'),
            'store_id'
        )->where(
            'page_id = ?',
            (int)$pageId
