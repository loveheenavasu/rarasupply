...
class Related extends AbstractModifier
{
    ...
    {
        $isRelatedproducts = $this->scopeConfig->getValue(
            'ced_csmarketplace/vproducts/relatedproducts',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
        $isUpsells = $this->scopeConfig->getValue(
            'ced_csmarketplace/vproducts/upsells',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
        $isCrosssells = $this->scopeConfig->getValue(
            'ced_csmarketplace/vproducts/crosssells',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
        $childrens = [];
        if($isRelatedproducts){
            $childrens[$this->scopePrefix . static::DATA_SCOPE_RELATED] = $this->getRelatedFieldset();
        }
        if($isUpsells){
            $childrens[$this->scopePrefix . static::DATA_SCOPE_UPSELL] = $this->getUpSellFieldset();
        }
        if($isCrosssells){
            $childrens[$this->scopePrefix . static::DATA_SCOPE_CROSSSELL] = $this->getCrossSellFieldset();
        }
        if(empty($childrens)){
            return $meta;
        }
