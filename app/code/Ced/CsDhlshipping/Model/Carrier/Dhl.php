...
namespace Ced\CsDhlshipping\Model\Carrier;;
...
class Dhl extends \Magento\Dhl\Model\Carrier
{
    ...
    public function collectRates(RateRequest $request)
    {
       ...
       try
       {
           if(!$this->_objectManager->create('Ced\CsMultishipping\Helper\Data')->isEnabled())
        {   die('not enable multi');
            return parent::collectRates($request);
        }
        if(!$this->_objectManager->create('Ced\CsDhlshipping\Helper\Data')->isEnabled())
        {    die('not dhl');
            return parent::collectRates($request);
        }
       ...
       }catch(\Exception $e){ echo $e->getMessage();die('exc---to');}
     ...
     */
     public function getAllowedMethod($vendor,$dhlSpecificConfig)
    {
        
        $contentType = $this->getConfigData('content_type');
        $allowedMethods = array();
        //$contentType=$dhlSpecificConfig['content_type'];
        // if($contentType=="1")
        // {
        ...
        // else {
        //     $allowedMethods=$dhlSpecificConfig['allowed_methods'];
    ...
    {   
        ...
        $vendor=array();
        $dhlSpecificConfig=array();
        if($vendorId!="admin")
            $dhlSpecificConfig = $this->_request->getVendorShippingSpecifics();
        if($vendorId!="admin"){
        ...
        try {
            ...
            echo $e->getMessage();die('++++in quote');
        }
        

        return $this->_parseResponseNew($responseBody,$vendor,$dhlSpecificConfig);
    }
    ...
    {   
        ...
        $client->setUri('http://xmlpitest-ea.dhl.com/XMLShippingServlet');
    ...
    protected function _buildQuotesRequestXml()
    {
        ...
        $nodeServiceHeader->addChild('SiteID', "xmlsellkorea");
        $nodeServiceHeader->addChild('Password',"9di5ObLUYs");
     ...
     * @SuppressWarnings(PHPMD.NPathComplexity)
     */
    protected function _parseResponseNew($response, \Ced\CsMarketplace\Model\Vendor $vendor, $dhlSpecificConfig)
    {   
        try
        {
        ...
        if (strlen(trim($response)) > 0) {
            if (strpos(trim($response), '<?xml') === 0) {
                ...
                print_r($xml->GetQuoteResponse->BkgDetails->QtdShp);die;//$this->parseXml($response, 'Magento\Shipping\Model\Simplexml\Element');
                if (is_object($xml)) {
                    if (in_array($xml->getName(), ['ErrorResponse', 'ShipmentValidateErrorResponse'])
                        ...
                        if (isset($xml->GetQuoteResponse->BkgDetails->QtdShp)) {
                            ...
                            $this->_addRates($quotedShipment,$vendor,$dhlSpecificConfig);
                            }
    ...
    try{ 
        ...
        $result = $this->_objectManager->get('Magento\Shipping\Model\Rate\Result');
        if ($this->_rates) {
            ...
            foreach ($this->_rates as $rate) {
                ...
                $rate =$this->_objectManager->get('\Magento\Quote\Model\Quote\Address\RateResult\Method');
                ...
                if($vendor->getVendorId())
                {$custom_method=$method.Ced\CsMultiShipping\Model\Shipping::SEPARATOR.$vendor->getVendorId();
     ...
     * @SuppressWarnings(PHPMD.CyclomaticComplexity)
     */
    public function _addRates(\SimpleXMLElement $shipmentDetails ,$vendor,$dhlSpecificConfig)
    {
        if (isset($shipmentDetails->ProductShortName)
            ...
            && array_key_exists((string)$shipmentDetails->GlobalProductCode, $this->getAllowedMethod($vendor,$dhlSpecificConfig))
        ) {
    ...
    protected function _showError()
    {
        ...
        if ($showMethod) {
            ...
            $error = $this->_objectManager->get('\Magento\Quote\Model\Quote\Address\RateResult\Error');
