        ...
        if (isset($dataSource['data']['items'])) {
            ...
            foreach ($dataSource['data']['items'] as & $item) {
                ...
                if ($item['payment_state']){
                    if ($item['payment_state'] == self::STATE_OPEN){
                        $buttonText = __('Pending');
                    }
                    if ($item['payment_state'] == self::STATE_PAID){
                        $buttonText = __('Paid');
                    }

                    if ($item['payment_state'] == self::STATE_CANCELED){
                        $buttonText = __('Canceled');
                    }
                    if ($item['payment_state'] == self::STATE_REFUNDED){
                        $buttonText = __('Refunded');
                    }

                    if ($item['order_payment_state'] == \Magento\Sales\Model\Order\Invoice::STATE_PAID
                        &&
                        $item['payment_state'] == self::STATE_OPEN)
                    {
                    ...
                    }elseif ($item['order_payment_state'] == \Magento\Sales\Model\Order\Invoice::STATE_PAID
                        &&
                        $item['payment_state'] == self::STATE_REFUND)
                    {

                        $url =  $this->urlBuilder->getUrl('csmarketplace/vpayments/new/',
                            array('vendor_id' => $item['vendor_id'], 'order_ids'=>$item['id'],
                                'type' => \Ced\CsMarketplace\Model\Vpayment::TRANSACTION_TYPE_DEBIT));
                        $buttonText .= $this->getRefundButtonHtml($url);
                        $item[$this->getData('name')] = $buttonText;
