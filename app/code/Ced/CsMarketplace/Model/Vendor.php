...
class Vendor extends \Ced\CsMarketplace\Model\AbstractModel
{
    ...
    public function getUrlPath() {
        if($this->moduleManager->isEnabled('Ced_CsSeoSuite') && !isNull($this->seoSuiteDataHelper)
    ...
    /**
     * @param array $data: custom attributes and values to validate
     * @return mixed
     */
    public function processCustomSaveData($data = []){
        return $this->getResource()->_processCustomSaveData($data, $this->setData($data));
    }
            ...
            if ($this->getData('shop_url')) {
                ...
                if (!$this->getId() ||
                    ...
                    if (strlen($this->getUrlPath()) == 0 &&
                        !isNull($this->seoSuiteDataHelper) &&
                        $this->seoSuiteDataHelper->isEnabled()
                    ) {
